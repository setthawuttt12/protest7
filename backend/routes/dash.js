    ...
    try {
        ...
        res.json({
            box: [
                ...
                {title:'แบบประเมินทั้งหมด',value: evaCount.total > 0 ? `${ (y.total*100/evaCount.total).toFixed(2)}%`: '00.00%'},
                {title:'แบบประเมินทั้งหมด',value: evaCount.total > 0 ? `${ (n.total*100/evaCount.total).toFixed(2)}%`: '00.00%'},
            ]
        })
    } catch (error) {
